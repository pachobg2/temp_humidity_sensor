...
| Update interval (retained command, HA select) | `home/<id>/update_interval`, `.../set` | `1 min` ... `60 min` (default `10 min`) |
...
| v3.7.0 | 2026-09-26 | New **Update Interval** select entity in HA (1, 2, 5, 10, 15, 30 or 60 min; default 10) choosing how long the device sleeps between readings. It is a retained, NVS-persisted command picked up on the next wake, so a change from HA takes up to one old interval to arrive and then applies to the sleep that follows; the current value is echoed back as state. `expire_after` on every HA entity now follows the chosen interval (3x) instead of the compile-time `EXPIRE_AFTER_SEC`, which is removed from `config.h`; when the interval changes, discovery is re-sent immediately so entities do not flap unavailable. `SLEEP_INTERVAL_US` in `config.h` is now just the default. |
